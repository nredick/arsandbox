...
void BathymetrySaverTool::postUpdate(void) const
	...
	if(!reply.isLiteral("HTTP")||!reply.isLiteral('/'))
		throw Misc::makeStdErr(0,"Not an HTTP reply!");
	reply.skipString();
	unsigned int statusCode=reply.readUnsignedInteger();
	if(statusCode!=200)
		throw Misc::makeStdErr(0,"HTTP error %d: %s",statusCode,reply.readLine().c_str());
	reply.readLine();
	reply.skipWs();
				...
				while(true)
					...
					if(coding=="chunked")
						...
						while(reply.isLiteral(';'))
							...
							reply.skipString();
							if(!reply.isLiteral('='))
								throw Misc::makeStdErr(0,"Malformed HTTP reply header");
							reply.skipString();
							}
						}
	...
	if(replyChunked)
		...
		while(true)
			...
			while(digit!='\r')
				digit=pipe->getChar();
			if(pipe->getChar()!='\n')
				throw Misc::makeStdErr(0,"Malformed HTTP chunk header");
			...
			if(pipe->getChar()!='\r'||pipe->getChar()!='\n')
				throw Misc::makeStdErr(0,"Malformed HTTP chunk footer");
			}
		...
		while(pipe->getChar()!='\r')
			...
			while(pipe->getChar()!='\r')
				;
			if(pipe->getChar()!='\n')
				throw Misc::makeStdErr(0,"Malformed HTTP body trailer");
			}
		if(pipe->getChar()!='\n')
			throw Misc::makeStdErr(0,"Malformed HTTP body trailer");
		}
